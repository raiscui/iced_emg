    ...
    fn or_insert_var_with_key<F: FnOnce() -> T, T: 'static + std::fmt::Debug>(
        ...
        match key {
            Some(k) => {
                ...
                if let Some(existing_secondary_map) = self.get_mut_secondarymap::<T>() {
...
fn insert_before_fn_common<T: 'static + std::clone::Clone>(
    sv: &StateVar<T>,
...
fn insert_after_fn_common<T: 'static + std::clone::Clone>(
    sv: &StateVar<T>,
    after_id: &StorageKey,
