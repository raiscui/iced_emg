...
pub fn map_to_motion(func: &impl Fn(&mut Motion), prop: &mut Property) {
    use Property::{Angle, Color, Exact, Path, Points, Prop, Prop2, Prop3, Prop4, Shadow};
    match prop {
        Exact(..) => (),

        Color(_, m) | Prop4(_, m) => m.iter_mut().for_each(func),
...
fn motion_is_done(motion: &Motion) -> bool {
    ...
    match running_interpolation {
        Interpolation::Spring { .. } => {
            motion.velocity.abs() < VELOCITY_ERROR_MARGIN
                && (motion.position - motion.target).abs() < position_error_margin
        }
        Interpolation::Easing(eased) => {
            //TODO check progress -1  or 1-progress right? use echo progress
            ((*eased.progress) - 1.).abs() < PROGRESS_ERROR_MARGIN
                || (eased.progress.abs() < PROGRESS_ERROR_MARGIN
                    && (motion.position - motion.target).abs() < position_error_margin)
        }
...
fn motion_is_done_for_cmd(motion: &Motion) -> bool {
    motion.velocity.is_zero()
        && (motion.position - motion.target).abs() < position_error_margin(motion)
}
fn is_done_sm(property: &Property) -> bool {
    ...
    match property {
        ...
        Prop4(_, m) | Color(_, m) => m.iter().all(motion_is_done),
...
fn step_interpolation_mut(dt: &Duration, motion: &mut Motion) {
    ...
    match interpolation_to_use {
        Interpolation::AtSpeed { per_second } => {
            let (new_pos, finished) = {
                if motion.position < motion.target {
                    let new = per_second.mul_add(dt.as_secs_f64().as_(), *motion.position);
            ...
            if finished {
                ...
                motion.velocity = per_second * 1000.;
                // pos/ms,  dis per millisecond
            };
        }
        Interpolation::Spring { stiffness, damping } => {
            let dt_sec = dt.as_secs_f64().as_();
            let f_spring = stiffness * (motion.target - motion.position);

            let f_damper = (damping * -1.) * motion.velocity;

            let new_velocity = (f_spring + f_damper).mul_add(dt_sec, *motion.velocity);
        ...
        Interpolation::Easing(easing) => {
            ...
            let new_progress: Precision = ((*progress)
                + AsPrimitive::<Precision>::as_((*dt).div_duration_f64(duration)))
            .min(1.);
            ...
            let new_velocity = if (new_progress - 1.).abs() < PROGRESS_ERROR_MARGIN {
                ...
                Duration::from_micros(unsafe {
                    let var_name: Precision =
                        (new_pos - motion.position.into_inner()).abs() * 1000.0;
                    var_name.round().to_u64().unwrap()

                    // var_name.round().to_int_unchecked()
                })
                .div_duration_f64(*dt)
                .as_()
            };
            ...
            } else if let Interpolation::Easing(e) = &mut motion.interpolation {
                e.progress = NotNan::new(new_progress).unwrap();
            }
        }
    }
...
#[cfg(test)]
mod tests_zip_all {
    use emg_common::{into_smvec, SmallVec};
...
fn set_target_mut(override_interpolation: bool, current: &mut Property, new_target: Property) {
    ...
    match (current, new_target) {
        ...
        (Prop4(_, box m), Prop4(_, box t)) | (Color(_, box m), Color(_, box t)) => {
            m.iter_mut().zip(t).for_each(set_motion_target);
        }
