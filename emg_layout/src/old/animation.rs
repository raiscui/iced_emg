...
// ────────────────────────────────────────────────────────────────────────────────
#[allow(dead_code)]
type SAPropsMessageStepsOG<Message> =
    StateAnchor<(Vector<PropertyOG>, Vector<Message>, Vector<StepOG<Message>>)>;

#[allow(dead_code)]
type SAPropsMessageSteps2OG<Message> = StateAnchor<(
    StepTimeVectorOG<Message>,
    Vector<StepOG<Message>>,
    Vector<PropertyOG>,
    Vector<Message>,
)>;
...
// ────────────────────────────────────────────────────────────────────────────────
#[derive(Debug, Clone)]
struct AnimationInsideOG<Message>
where
    Message: Clone + std::fmt::Debug + 'static,
{
    pub(crate) steps: StateVar<Vector<StepOG<Message>>>,
    pub(crate) interruption: StateVar<StepTimeVectorOG<Message>>,
    pub(crate) props: Vector<StateVarPropertyOG>,
}

impl<Message> AnimationInsideOG<Message>
where
    Message: Clone + std::fmt::Debug + 'static,
{
    #[topo::nested]
    fn new_in_topo(props: Vector<StateVarPropertyOG>) -> Self {
        props
            .iter()
            .for_each(|prop| prop.set_with_once(|p| set_default_interpolation_og(p.clone())));
        Self {
    ...
    ( $( $element:expr ) , * ) => {
        {
            ...
            $crate::old::AnimationEOG::new_in_topo(v)
    ...
    // Ix: Clone + std::hash::Hash + Eq + Default + Ord + 'static,
{
    ...
    // processed_interruptions: StateAnchor<(StepTimeVectorOG<Message>, StepTimeVectorOG<Message>)>,
    // revised: SAPropsMessageSteps2<Message>,
    id: TopoKey,
    ref_count: Rc<Cell<usize>>,
}

impl<Message> Clone for AnimationEOG<Message>
where
    Message: Clone + std::fmt::Debug + 'static + PartialEq,
{
    fn clone(&self) -> Self {
...
impl<Message> std::fmt::Debug for AnimationEOG<Message>
where
    Message: Clone + std::fmt::Debug + 'static + PartialEq,
    // Ix: Clone + std::hash::Hash + Eq + Default + Ord + 'static,
{
...
impl<Message> AnimationEOG<Message>
where
    Message: Clone + std::fmt::Debug + 'static + PartialEq,
{
        ...
        Ix: Clone + std::hash::Hash + Eq + Default + Ord + std::fmt::Display + 'static,
    {
        edge.build_path_layout(|mut l| {
            ...
            self.inside.props.iter().for_each(|svp| {
                ...
                let name = svp.get_with(emg_animation::models::PropertyOG::name);
    ...
    #[must_use]
    pub fn new_in_topo(
        props: Vector<StateVarPropertyOG>,
        // sv_now: StateVar<Duration>,
        ...
        // Ix: Clone + std::hash::Hash + Eq + Default + Ord + std::fmt::Display + 'static,
    {
        ...
        let sv_inside: AnimationInsideOG<Message> =
            AnimationInsideOG::<Message>::new_in_topo(props);

        // let store = rc_store2.borrow();

        let sa_timing = Self::set_timer(sv_now);

        let AnimationInsideOG {
        ...
        let i_p_cut = {
            ...
            // let mut opt_old_interruption: Option<StepTimeVectorOG<Message>> = None;
            // interruption_init.store_watch(&store)
            let pa: StateAnchor<Vector<PropertyOG>> = props_init
                .iter()
        ...
        let revised: SAPropsMessageSteps2OG<Message> = (&sa_timing, &i_p_cut, &steps_init.watch())
            .map(
                move |timing: &Timing,
                      (interruption, props): &(StepTimeVectorOG<Message>, Vector<PropertyOG>),
                    ...
                    let (revised_props, sent_messages, revised_steps) =
                        resolve_steps_og(new_props, new_steps, timing.dt());
                    (
...
impl<Message> Drop for AnimationEOG<Message>
where
    Message: Clone + std::fmt::Debug + 'static + PartialEq,
{
    fn drop(&mut self) {
...
mod tests {
    ...
    use crate::old::animation::global_clock;
    use crate::old::{EPath, EdgeItemNode, EmgEdgeItem, GraphEdgesDict};

    use super::AnimationEOG;
    ...
    fn nom_am_run(am: &mut emg_animation::models::AnimationOG<Message>) {
        interrupt_og(
            vector![
        ...
        for i in 1002..2000 {
            emg_animation::update_og(Tick(Duration::from_millis(i * 16)), am);
    ...
    fn many_am_run(
        ...
        assert_eq!(a.inside.props[0].get(), PropertyOG::from(width(px(0))));
        // a.inside.props[0].store_get(storeref);
    }
    ...
    fn test_layout_anima() {
        ...
        insta::with_settings!({snapshot_path => Path::new("./layout_am")}, {
            ...
            let a: AnimationEOG< Message> =
                // AnimationEdge::new_in_topo(into_vector![width(px(1))], e1, sv_now);
                AnimationEOG::new_in_topo(into_vector![css_w]);
    ...
    fn anima_macro_for_bench() {
        ...
        sv_now.set(Duration::from_millis(0));
        let css_w: StateVar<CssWidth> = use_state(width(px(1)));
        let a: AnimationEOG<Message> = anima_og![css_w];
